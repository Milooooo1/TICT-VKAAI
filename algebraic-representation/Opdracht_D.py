...
Doordat de convolutie lagen met zijn filters bepaalde features uit een afbeelding weet te trekken
en hierop getrained wordt met de data wordt ervoor gezorgd dat de output laag elke filter bij een 
class hoort. Hierdoor ontstaat het efect dat het eerste filter gericht zoekt naar features voor
het cijfer 0. Je ziet dan ook in afbeelding A_1 dat de plekken waarop er een 0 staat de features
duidelijker zijn afgebeeld met een gele kleur, met andere woorden het netwerk is er zekerder van dat
op die plekken er een 0 staat.

De pooling layers bepalen in grote lijnen de output van de bitmaps omdat deze layers de input
shape van de afbeelding verkleinen. Als je een filter van 2x2 hebt halveert de pooling layer het
aantal inputs.   
"""
...
Per output filter kan je een threshold waarde instellen waarbij de je zeker genoeg weet dat iets
ook daadwerkelijk de juiste cijfer is. Wanneer je dit gedaan hebt kan je alles onder deze threshold
weg filteren en gaan kijken naar de patronen en hier bounding boxes omheen trekken, deze coordinaten
zet je vervolgens in een lijst.
...
er toch voor dat het meer zal zijn dan 4.7 us. 
